...
const CalendarApp = () => {
	...
	const editEvent = (event_id) => {
		...
		const eventToEdit = events.find(
			(event) => event.event_id === Number(event_id)
		);
	...
	const handleSave = () => {
		...
		fetch(
			`http://localhost:8080/events/${community_id}/${updatedEvent.event_id}`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					...
					<ul className='events'>
						{events.map((event, index) => (
							<li key={index} className='event-item'>
								...
								{new Date(event.date).toDateString()} - {event.title} at{" "}
								{event.location} <br /> {event.description}
